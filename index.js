...
const result = a.map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
console.log(result);
//№4
...
const negativeProduct = array.filter(value => value < 0).reduce((product, value) => product * value, 1);
console.log('Произведение отрицательных элементов массива:', negativeProduct);
//№6
const arraay = [24, 35, 29, 44, 8, 22, 4];

const inversions = array.map((value, index) => {
    return array.slice(index + 1).filter(nextValue => value > nextValue).length;
...
console.log('Игроки одинакового роста:', sameHeightPlayers.length > 0 ? sameHeightPlayers : 'Нет игроков одинакового роста');
//№8
const arrayyy = [1, 2, 3, 4, 5, 6, 7, 8, 9, 1];

const hasDuplicates = arr.reduce((acc, value, index, array) => {
...
console.log('Есть ли в массиве совпадающие элементы:', hasDuplicates ? 'Да' : 'Нет');
//№9
const students = [
